...
class Trainer:
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    for _ in range(NUM_MARGINALS):
                        ...
                        inputs = self._prepare_inputs(inputs, force_regenerate=True, instruction = instruction)
                        marginal_data.append(inputs)
                    for _ in range(self.num_iterations):
                        with context():
                            all_losses = []
                            for inputs in marginal_data:    
                                tr_loss_step = self.training_step(model, inputs, num_items_in_batch)
                                print("tr_loss_step", tr_loss_step)
                                all_losses.append(tr_loss_step)
                            print("all_losses", all_losses)
                            losses = torch.stack(all_losses)         # [5, 3]
                            print("losses", losses)
                            logical_losses = losses.mean(dim=0)      # [3]
                            print("logical_losses",logical_losses)
                            loss = logical_losses.mean() 
                            print("LOSS", loss)
                        ########################################################################################
                            kwargs = {}
                            ...
                            tr_loss_step = loss.detach()                  # Safely log this
                            del all_losses, losses, logical_losses, loss  # Clean up graph references
                        ...
                        if self.control.should_epoch_stop or self.control.should_training_stop:
                            if is_torch_xla_available():
                                xm.mark_step()
                            break
                    del marginal_data,tr_loss_step, inputs
                    torch.cuda.empty_cache()
    ...
    def training_step(
        ...
        with self.compute_loss_context_manager():
            ...
            #print("loss_training_step", loss)
        ###########################################################################################################################################
        ##  MOdify to handle marginalization
        # NUM_MARGINALS = 2
        # instruction = self.env.agent.start_session()
        # all_losses = []
        # for _ in range(NUM_MARGINALS):
        ...
        #     if is_sagemaker_mp_enabled():
        #         loss_mb = smp_forward_backward(model, inputs, self.args.gradient_accumulation_steps)
        #         all_losses.append(loss_mb.reduce_mean().detach().to(self.args.device))
        #         continue

        #     with self.compute_loss_context_manager():
        #         compute_loss = self.compute_loss(model, inputs, num_items_in_batch=num_items_in_batch)
        #         all_losses.append(compute_loss)
        #         #print("loss_training_step", loss)
        # print("all_losses", all_losses)
        # losses = torch.stack(all_losses)         # [5, 3]
        # print("losses", losses)
